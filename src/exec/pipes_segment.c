...
/*   Updated: 2025/08/11 19:47:36 by pede-jes         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	execute_child_process(t_token **segments, int **pipes, int pipe_count, int i, t_env *envs)
{
...
void	create_child_processes(t_token **segments, int **pipes, pid_t *pids, t_env *envs)
{
    ...
    while (i <= pipe_count)
    {
        pids[i] = fork();
        if (pids[i] == 0)
            execute_child_process(segments, pipes, pipe_count, i, envs);
        i++;
    }
}
