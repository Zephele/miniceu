...
/*   Updated: 2025/08/20 15:34:07 by ratanaka         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_token	*exec_single_pipe(t_token *left_tokens, t_token *right_tokens, t_env *envs)
{
	...
	pid_t	pid1;
	pid_t	pid2 = 0;
	t_token	*heredoc_token_l;
	t_token	*heredoc_token_r;
	...
	heredoc_token_l = find_heredoc_token(left_tokens);
	if (heredoc_token_l)
	{
		gg()->heredoc_file = ft_read_heredoc(heredoc_token_l->next->content, 0, NULL);
		if (!gg()->heredoc_file)
			return (NULL);
	}
	pid1 = fork();
	...
	heredoc_token_r = find_heredoc_token(right_tokens);
	if (heredoc_token_r)
	{
		if (gg()->heredoc_file)
			free (gg()->heredoc_file);
		gg()->heredoc_file = ft_read_heredoc(heredoc_token_r->next->content, 0, NULL);
		if (!gg()->heredoc_file)
			return (NULL);
	}
	pid2 = fork();
	if (pid2 == 0)
	{
		if (heredoc_token_r && gg()->heredoc_file)
		{
			close(pipefd[0]);
			int fd = open(gg()->heredoc_file, O_RDONLY);
			if (fd >= 0)
